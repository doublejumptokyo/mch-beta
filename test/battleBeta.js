...
contract('BattleBeta', function(accounts) {
    ...
    it("setHeroManagerAddress to heroAsset", function () {
        return HeroManager.deployed().then(async function (hm) {
            return HeroAsset.deployed().then(async function (hs) {
                await hs.setHeroManagerAddress(
                    hm.address,
                    { from: accounts[0] });
    ...
    it("setItemAddress to manager", function () {
        return ItemManager.deployed().then(async function (im) {
            return Item.deployed().then(async function (instance) {
                await im.setItemAddress(
                    instance.address,
                    { from: accounts[0] });
                return im.item();
            }).then(function (val) {
    ...
    it("setItemAssetAddress to manager", function () {
        return ItemManager.deployed().then(async function (im) {
            return ItemAsset.deployed().then(async function (ia) {
                await im.setItemAssetAddress(
                    ia.address,
                    { from: accounts[0] });
                return im.itemAsset();
            }).then(function (val) {
    ...
    it("setItemManagerAddress to Item", function () {
        return ItemManager.deployed().then(async function (im) {
            return Item.deployed().then(async function (instance) {
                await instance.setItemManagerAddress(
                    im.address,
                    { from: accounts[0] });
    ...
    it("setItemManagerAddress to itemAsset", function () {
        return ItemManager.deployed().then(async function (im) {
            return ItemAsset.deployed().then(async function (ia) {
                await ia.setItemManagerAddress(
                    im.address,
                    { from: accounts[0] });
...
it("battle transaction set", function () {
	return BattleTransaction.deployed().then(async function (bt) {
		return BattleBeta.deployed().then(async function (bb) {
			await bb.setBattleTransactionAddress(
				bt.address,
				{ from: accounts[0] });
...
it("setPartyTest", async function () {
	...
	await BattleBeta.deployed().then(async function (bb) {
		await bb.setParty(
				input1,
				input2,
				input3,
