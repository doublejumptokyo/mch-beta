...
contract('Land', function(accounts) {
    ...
    return LandManager.deployed().then(async function(im) {
      return Land.deployed()
        .then(async function(instance) {
          await im.setLandAddress(instance.address, { from: accounts[0] })
          return im.land()
        })
        .then(function(val) {
  ...
  it('setLandAssetAddress to manager', function() {
    return LandManager.deployed().then(async function(im) {
      return LandAsset.deployed()
        .then(async function(ia) {
          await im.setLandAssetAddress(ia.address, { from: accounts[0] })
          return im.landAsset()
        })
        .then(function(val) {
  ...
  it('setLandManagerAddress to Land', function() {
    return LandManager.deployed().then(async function(im) {
      return Land.deployed()
        .then(async function(instance) {
          await instance.setLandManagerAddress(im.address, {
            from: accounts[0]
          })
  ...
  it('createLand', function() {
    return LandManager.deployed().then(async function(lm) {
      return Land.deployed()
        .then(async function(instance) {
          await lm.createLand(landType, accounts[0], { from: accounts[0] })
