...
contract('BattleTest1', function(accounts) {
    ...
    it('setUnit', async function() {
        await test.setUnit(0, 450, 100, 50, 50, 3, 2, 3, 100, 1001, 1002, 1003, battleId);
        await test.setUnit(1, 300, 120, 30, 50, 2, 3, 2, 100, 2001, 2002, 2003, battleId);
        await test.setUnit(2, 300, 50, 100, 50, 11, 12, 21, 100, 3001, 3002, 3003, battleId);
        await test.setUnit(3, 500, 100, 50, 50, 3, 3, 3, 100, 4001, 4002, 4003, battleId);
        await test.setUnit(4, 400, 130, 40, 30, 2, 2, 2, 100, 5001, 5002, 5003, battleId);
        await test.setUnit(5, 300, 20, 150, 30, 12, 11, 22, 100, 6001, 6002, 6003, battleId);
    });
	...
	it('nexts', async function() {
		...
		for(i=0; i<100; i++) {
            ...
            for(j=0; j<7; j++) {
                if(action.logs[0].args.effectPositions[j]) result.effectPositions.push(j);
            }
