  ...
  describe('as an escrow', function() {
    describe('deposits', function() {
      it('can accept a single deposit', async function() {
        ...
        ;(await ethGetBalance(this.escrow.address)).should.be.bignumber.equal(
          amount
        )
        ;(await this.escrow.depositsOf(payee1)).should.be.bignumber.equal(
          amount
        )
      })
      ...
      it('emits a deposited event', async function() {
        const receipt = await this.escrow.deposit(payee1, {
          from: primary,
          value: amount
        })

        const event = expectEvent.inLogs(receipt.logs, 'Deposited', {
          payee: payee1
        })
      ...
      it('can track deposits to multiple accounts', async function() {
        ...
        ;(await this.escrow.depositsOf(payee2)).should.be.bignumber.equal(
          amount * 2
        )
      })
    })
    ...
    describe('withdrawals', async function() {
      ...
      it('emits a withdrawn event', async function() {
        ...
        const event = expectEvent.inLogs(receipt.logs, 'Withdrawn', {
          payee: payee1
        })
        event.args.weiAmount.should.be.bignumber.equal(amount)
      })
