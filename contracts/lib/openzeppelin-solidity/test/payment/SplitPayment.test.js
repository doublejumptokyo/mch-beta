...
contract('SplitPayment', function([
  _,
  owner,
  payee1,
  payee2,
  ...
  it('rejects more payees than shares', async function() {
    await expectThrow(
      SplitPayment.new([payee1, payee2, payee3], [20, 30]),
      EVMRevert
    )
  })
  ...
  it('rejects more shares than payees', async function() {
    await expectThrow(
      SplitPayment.new([payee1, payee2], [20, 30, 40]),
      EVMRevert
    )
  })
  ...
  it('rejects null payees', async function() {
    await expectThrow(
      SplitPayment.new([payee1, ZERO_ADDRESS], [20, 30]),
      EVMRevert
    )
  })
  ...
  context('once deployed', function() {
    ...
    it('should have total shares', async function() {
      ;(await this.contract.totalShares()).should.be.bignumber.equal(
        20 + 10 + 70
      )
    })
    ...
    it('should accept payments', async function() {
      ...
      ;(await ethGetBalance(this.contract.address)).should.be.bignumber.equal(
        amount
      )
    })
    ...
    it('should distribute funds to payees', async function() {
      ...
      ;(await this.contract.totalReleased()).should.be.bignumber.equal(
        initBalance
      )
    })
  })
