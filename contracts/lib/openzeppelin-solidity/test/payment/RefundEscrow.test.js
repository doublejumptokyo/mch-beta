...
contract('RefundEscrow', function([
  _,
  primary,
  beneficiary,
  refundee1,
  ...
  it('requires a non-null beneficiary', async function() {
    await expectThrow(RefundEscrow.new(ZERO_ADDRESS, { from: primary }))
  })
  ...
  context('once deployed', function() {
    ...
    context('active state', function() {
      ...
      it('accepts deposits', async function() {
        ...
        ;(await this.escrow.depositsOf(refundee1)).should.be.bignumber.equal(
          amount
        )
      })
    ...
    context('closed state', function() {
      beforeEach(async function() {
        await Promise.all(
          refundees.map(refundee =>
            this.escrow.deposit(refundee, { from: primary, value: amount })
          )
        )
      ...
      it('rejects deposits', async function() {
        await expectThrow(
          this.escrow.deposit(refundee1, { from: primary, value: amount }),
          EVMRevert
        )
      })
      ...
      it('prevents entering the refund state', async function() {
        await expectThrow(
          this.escrow.enableRefunds({ from: primary }),
          EVMRevert
        )
      })
