...
contract('TokenVesting', function([_, owner, beneficiary]) {
  ...
  it('rejects a duration shorter than the cliff', async function() {
    ...
    await expectThrow(
      TokenVesting.new(beneficiary, this.start, cliffDuration, duration, true, {
        from: owner
      })
    )
  })
  ...
  context('once deployed', function() {
    ...
    it('can get state', async function() {
      ...
      ;(await this.vesting.cliff()).should.be.bignumber.equal(
        this.start + this.cliffDuration
      )
    ...
    it('cannot be released before cliff', async function() {
      await expectThrow(this.vesting.release(this.token.address), EVMRevert)
    })
    ...
    it('should release proper amount after cliff', async function() {
      ...
      ;(await this.vesting.released(this.token.address)).should.bignumber.equal(
        releasedAmount
      )
    })
    ...
    it('should linearly release tokens during vesting period', async function() {
      ...
      for (let i = 1; i <= checkpoints; i++) {
        const now =
          this.start + this.cliffDuration + i * (vestingPeriod / checkpoints)
        ...
        ;(await this.vesting.released(
          this.token.address
        )).should.bignumber.equal(expectedVesting)
      }
    })
    ...
    it('should return the non-vested tokens when revoked by owner', async function() {
      ...
      await this.vesting.revoke(this.token.address, { from: owner })
      ;(await this.token.balanceOf(owner)).should.bignumber.equal(
        amount.sub(vested)
      )
    })
