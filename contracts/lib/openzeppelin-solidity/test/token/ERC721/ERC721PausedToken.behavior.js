...
function shouldBehaveLikeERC721PausedToken(owner, [recipient, operator]) {
  ...
  describe('like a paused ERC721', function() {
    ...
    it('reverts when trying to approve', async function() {
      await assertRevert(
        this.token.approve(recipient, firstTokenId, { from: owner })
      )
    })
    ...
    it('reverts when trying to transferFrom', async function() {
      await assertRevert(
        this.token.transferFrom(owner, recipient, firstTokenId, { from: owner })
      )
    })

    it('reverts when trying to safeTransferFrom', async function() {
      await assertRevert(
        this.token.safeTransferFrom(owner, recipient, firstTokenId, {
          from: owner
        })
      )
    })
