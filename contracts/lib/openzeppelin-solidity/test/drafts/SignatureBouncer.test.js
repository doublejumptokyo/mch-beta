...
contract('SignatureBouncer', function([
  _,
  signer,
  otherSigner,
  anyone,
  ...
  describe('modifiers', function() {
    context('plain signature', function() {
      it('allows valid signature for sender', async function() {
        await this.sigBouncer.onlyWithValidSignature(
      ...
      it('does not allow invalid signature for sender', async function() {
        await assertRevert(
          this.sigBouncer.onlyWithValidSignature(INVALID_SIGNATURE, {
            from: authorizedUser
          })
        )
      })
      ...
      it('does not allow valid signature for method for sender', async function() {
        await assertRevert(
          this.sigBouncer.onlyWithValidSignature(
            this.signFor(authorizedUser, 'onlyWithValidSignature'),
            { from: authorizedUser }
          )
        )
      })
    ...
    context('method signature', function() {
      ...
      it('does not allow invalid signature with correct method for sender', async function() {
        await assertRevert(
          this.sigBouncer.onlyWithValidSignatureAndMethod(INVALID_SIGNATURE, {
            from: authorizedUser
          })
        )
      })
    ...
    context('method and data signature', function() {
      it('allows valid signature with correct method and data for sender', async function() {
        await this.sigBouncer.onlyWithValidSignatureAndData(
          UINT_VALUE,
          this.signFor(authorizedUser, 'onlyWithValidSignatureAndData', [
            UINT_VALUE
          ]),
          { from: authorizedUser }
        )
      ...
      it('does not allow invalid signature with correct method and data for sender', async function() {
        await assertRevert(
          this.sigBouncer.onlyWithValidSignatureAndData(
            UINT_VALUE,
            INVALID_SIGNATURE,
            { from: authorizedUser }
          )
  ...
  context('signature validation', function() {
    context('plain signature', function() {
      ...
      it('does not validate valid signature for method for valid user', async function() {
        ;(await this.sigBouncer.checkValidSignature(
          authorizedUser,
          this.signFor(authorizedUser, 'checkValidSignature')
        )).should.equal(false)
      })
    })
    ...
    context('method signature', function() {
      it('validates valid signature with correct method for valid user', async function() {
        ;(await this.sigBouncer.checkValidSignatureAndMethod(
          authorizedUser,
    ...
    context('method and data signature', function() {
      it('validates valid signature with correct method and data for valid user', async function() {
        ;(await this.sigBouncer.checkValidSignatureAndData(
          authorizedUser,
          BYTES_VALUE,
          UINT_VALUE,
