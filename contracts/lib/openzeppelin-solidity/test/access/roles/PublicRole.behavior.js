...
function shouldBehaveLikePublicRole(
  authorized,
  otherAuthorized,
  [anyone],
  rolename
  ...
  describe('should behave like public role', function() {
    beforeEach('check preconditions', async function() {
      ...
      ;(await this.contract[`is${rolename}`](otherAuthorized)).should.equal(
        true
      )
    ...
    describe('add', function() {
      ...
      it(`emits a ${rolename}Added event`, async function() {
        const { logs } = await this.contract[`add${rolename}`](anyone, {
          from: authorized
        })
      ...
      it('reverts when adding role to the null account', async function() {
        await assertRevert(
          this.contract[`add${rolename}`](ZERO_ADDRESS, { from: authorized })
        )
      })
    })
