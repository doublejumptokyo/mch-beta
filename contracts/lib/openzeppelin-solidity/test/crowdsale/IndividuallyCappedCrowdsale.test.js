...
const IndividuallyCappedCrowdsaleImpl = artifacts.require(
...
contract('IndividuallyCappedCrowdsale', function([
  ...
  beforeEach(async function() {
    ...
    this.crowdsale = await IndividuallyCappedCrowdsaleImpl.new(
      rate,
      wallet,
      this.token.address,
      { from: capper }
  ...
  describe('individual caps', function() {
    ...
    it('reverts when a non-capper sets a cap', async function() {
      await expectThrow(
        this.crowdsale.setCap(alice, capAlice, { from: anyone }),
        EVMRevert
      )
    })
    ...
    context('with individual caps', function() {
      ...
      describe('accepting payments', function() {
        ...
        it('should reject payments outside cap', async function() {
          ...
          await expectThrow(
            this.crowdsale.buyTokens(alice, { value: 1 }),
            EVMRevert
          )
        })
        ...
        it('should reject payments that exceed cap', async function() {
          await expectThrow(
            this.crowdsale.buyTokens(alice, { value: capAlice.plus(1) }),
            EVMRevert
          )
          await expectThrow(
            this.crowdsale.buyTokens(bob, { value: capBob.plus(1) }),
            EVMRevert
          )
        })
        ...
        it('should manage independent caps', async function() {
          await this.crowdsale.buyTokens(alice, { value: lessThanCapAlice })
          await expectThrow(
            this.crowdsale.buyTokens(bob, { value: lessThanCapAlice }),
            EVMRevert
          )
        })
        ...
        it('should default to a cap of zero', async function() {
          await expectThrow(
            this.crowdsale.buyTokens(charlie, { value: lessThanCapBoth }),
            EVMRevert
          )
        })
      })
      ...
      describe('reporting state', function() {
        it('should report correct cap', async function() {
          ;(await this.crowdsale.getCap(alice)).should.be.bignumber.equal(
            capAlice
          )
        })
        ...
        it('should report actual contribution', async function() {
          await this.crowdsale.buyTokens(alice, { value: lessThanCapAlice })
          ;(await this.crowdsale.getContribution(
            alice
          )).should.be.bignumber.equal(lessThanCapAlice)
        })
      })
    })
  })
