...
contract('Crowdsale', function([_, investor, wallet, purchaser]) {
  ...
  it('requires a non-null token', async function() {
    await assertRevert(Crowdsale.new(rate, wallet, ZERO_ADDRESS))
  })
  ...
  context('with token', async function() {
    ...
    it('requires a non-zero rate', async function() {
      await assertRevert(Crowdsale.new(0, wallet, this.token.address))
    })

    it('requires a non-null wallet', async function() {
      await assertRevert(Crowdsale.new(rate, ZERO_ADDRESS, this.token.address))
    })
      ...
      describe('accepting payments', function() {
        describe('bare payments', function() {
          ...
          it('reverts on zero-valued payments', async function() {
            await assertRevert(this.crowdsale.send(0, { from: purchaser }))
          })
        })
